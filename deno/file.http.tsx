...
// Helper function to detect if file is HTTP type
function isHttpFile(filename: string, valType?: string): boolean {
  return valType === "http" || valType === "script";
}
      ...
      <html lang="en">
        <head>
          ...
          <style>
            ...
            .http-indicator { @apply bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs font-medium; }
          `}
          </style>
        </head>
...
app.get("/:zon/:file/raw", async (c: Context) => {
  ...
  try {
    // Get file metadata to check if it's HTTP type
    const fileKey = `${zon}:${file}`;
    const fileData = await kv.get(`file:${fileKey}`);

    if (!isHttpFile(file, fileData?.type)) {
      return c.json({ error: `File '${file}' is not an HTTP file` }, 400);
    }
    ...
    return c.json({ content, file, zon, type: "http" });
  } catch (error) {
    console.error("Error fetching HTTP file:", error);
    return c.json({ error: "Failed to fetch HTTP file content" }, 500);
  }
});

// HTTP file editor view
app.get("/:zon/:file", async (c: Context) => {
  ...
  try {
    ...
    if (!isHttpFile(file, valType)) {
      return c.render(
        <div className="container mx-auto px-4 py-8">
          <div className="text-center">
            <h1 className="text-2xl font-bold text-gray-900 mb-4">Not an HTTP File</h1>
            <p className="text-gray-600 mb-4">The file "{file}" is not an HTTP file type.</p>
    ...
    const fileType = FILE_TYPES.http;
    ...
    return c.render(
      <div className="min-h-screen bg-gray-50">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="http-indicator">HTTP</span>
                    </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <button
                  ...
                  onclick={`window.open('https://${zon}.val.run', '_blank')`}
                >
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="bg-white rounded-lg shadow-lg overflow-hidden file-type-http">
            <div className="bg-green-50 px-4 py-2 border-b border-green-200">
              <div className="flex items-center space-x-2">
                <i data-lucide="globe" className="h-4 w-4 text-green-600"></i>
                <span className="text-sm font-medium text-green-800">HTTP Endpoint</span>
                <span className="text-xs text-green-600">• Handles web requests and responses</span>
              </div>
            </div>
    ...
    console.error("Error loading HTTP file editor:", error);
    return c.render(
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Error</h1>
          <p className="text-gray-600 mb-4">Failed to load HTTP file editor.</p>
          <a href={config.urls.zons} className="text-blue-600 hover:text-blue-800">← Back to Gallery</a>
        </div>
      </div>,
    );
...
const authorization = createMiddleware(async function(c: Context) {
  // Authorization disabled for development// file is not realy saved
  return ; 
});

// Save HTTP file endpoint
app.post("/:zon/:file", authorization, async (c: Context) => {
  ...
  try {
    // Verify it's an HTTP file
    const fileKey = `${zon}:${file}`;
    const fileData = await kv.get(`file:${fileKey}`);

    if (!isHttpFile(file, fileData?.type)) {
      return c.json({ error: "File is not an HTTP file" }, 400);
    }

    // Save file content to KV
    await kv.set(`content:${fileKey}`, content);

    return c.json({ success: true, message: "HTTP file saved successfully" });
  } catch (error: unknown) {
    console.error("Error saving HTTP file:", error);
