  ...
  if (!zon) {
    ...
    markup = renderToStaticMarkup(
      <section className="view editor-view" aria-labelledby="editor-title">
        ...
        {detailType ? <div className="type-detail-slot" {...{ "hx-get": `/_fragments/files/${detailType}/${encodedZon}/${encodedPath}`, "hx-trigger": "load", "hx-swap": "innerHTML" }}><p>Loading {detailType} details…</p></div> : <aside className="type-detail" aria-label="Generic file details"><div className="type-detail-icon"><File aria-hidden="true" /></div><div className="type-detail-copy"><p className="eyebrow">Shared file</p><h2>Generic Y.Text source</h2><p>This file has no type-specific runtime details. It uses the same collaborative editor as every registry file.</p></div><div className="type-detail-meta"><span>Plain text</span><span>{type || "file"}</span></div></aside>}
